...
DynamicArray<T>::DynamicArray( ) : count_(0), capacity_(INIT_CAPACITY) {
    elements_ = static_cast<T*>(
        memory::memory_manager.reallocate(nullptr, 0, capacity_ * sizeof(T)));
}
...
DynamicArray<T>::~DynamicArray( ) {
    memory::memory_manager.reallocate(elements_, sizeof(T) * capacity_, 0);
}
...
inline void DynamicArray<T>::resize( ) {
    ...
    elements_ = static_cast<T*>(
        memory::memory_manager.reallocate(elements_, old_size, new_size));
}
